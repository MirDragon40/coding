...
public class CoroutineExample1 : MonoBehaviour
{
    ...
    // 작업 하나하나를 루틴이라고 하기도 함
    void Start()
    {
        // 동기란: 어떤 작업을 수행할 때, 그 작업을 처음부터 끝까지 멈추지 않고 다 수행하는 것
        // -> 하나의 코드 실행이 완료되어야 다음 코드가 실행된다.
        // e.g.) 정수빈.아이스크림사기();  // 20분 걸림
        // 장점: 작업 순서를 명확하게 보장하고, 작업이 끝날 때까지 결과를 기다리는 것이 가능하다.

        // 비동기란: 어떤 작업을 수행할 떄 그 작업이 완료되지 않더라도 다음 코드를 실행하는 방식
        // -> 즉, 작업 완료를 기다리지 않고 다음 코드를 실행하는 것
        // 장점: 작업이 오래 걸리는 경우 시간을 절약하고, 병렬적인 작업 처리가 가능하다.
        // 김홍일.아이스크림 사오라고 시키기.(정수빈);
        // 김홍일.수업진행();
    ...
    // 코루틴의 특징:
    // 1. 비동기 실행이 가능하다. 
    // 2. 세밀한 실행 제어가 가능하다. (시간적인 면에서, 시간: 대기, 조절, 일시중지 등 많은 기능)
