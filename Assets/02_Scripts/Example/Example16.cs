...
public class Example16 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (health <= 0)       // 체력이 0 이하가 되면 "게임오버"라는 텍스트를 출력
        {
            Debug.Log("게임오버");
        }
        else
        ...
        // 온도가 12도 이하가 되면 "한파주의보"라는 텍스트를 출력
        if (temperature <= 12)
        {
            Debug.Log("한파주의보");
        }
        ...
        //(temperature > 12 && temperature < 36)  // 옆의 조건은 if 와 함께 사용한다.
        {
            Debug.Log("정상 온도");
        }

        // 현재 온도 알려주는 텍스트 출력 : "현재온도는 00도 입니다."
        Debug.Log($"현재온도는 {temperature}도 입니다.");
